...
class PredictPipeline:
    ...
    def preprocess_input(self, input_df, preprocessor, feature_means):
        try:
            ...
            numeric_cols = preprocessor['numerical_cols']
            categorical_cols = preprocessor['categorical_cols']
            X_num = preprocessor['numerical_pipeline'].transform(input_df[numeric_cols])
            X_cat = pd.get_dummies(input_df[categorical_cols], drop_first=True)
            X_cat = X_cat.reindex(columns=preprocessor['categorical_cols'], fill_value=0)
